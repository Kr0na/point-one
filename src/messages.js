...
export const PROVIDE_KEY =  'Your %s %s initialize without key, please key to constructor to prevent errors with build by uglify or something like that. Just for test we set key as class name'
...
export const STORE_METHOD_NOT_FOUND = 'Method %s for %s not found in store %s'
...
export function warn(message, ...params) {
    if (process.env.NODE_ENV !== 'production') {
        console.warn(sprintf(message, ...params))
    }
}

export function err(message, ...params) {
    if (process.env.NODE_ENV !== 'production') {
        throw new Error(sprintf(message, ...params))
    }
}

export function info(message) {
    if (process.env.NODE_ENV === "development") {
        console.info(sprintf(message, ...params))
    }
}